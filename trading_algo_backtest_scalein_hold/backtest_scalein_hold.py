"""
Backtest QQQ - Scale-in with Fixed Hold
...
Hold portion:
- `hold_pct` (default 0.5) fraction of initial capital is bought on the first bar after warmup
  and held permanently until end of backtest (never sold).

Dynamic portion:
- The remaining (1 - hold_pct) fraction of capital is allocated dynamically using the
  scale-in bin strategy: buy at drop thresholds from prior peak, exit when
  Close > sma_max_at_entry AND Close < current SMA_exit.

Exit (dynamic trades only):
- Track sma_running_max frozen at trade entry (sma_max_at_entry).
- Exit when Close > sma_max_at_entry AND Close < current SMA_exit (both required).

Parameters:
- hold_pct: fraction of equity permanently held (default 0.5)
- bin_size_pct, max_drop_pct, initial_alloc_pct: scale-in bin settings
- exit_sma_window: SMA window for exit signal
"""
...
def check_entry(close, peak, thresholds, cumul, allocated_cum_for_peak, eq, cash):
    """
    Returns (should_enter, invest_amount, desired_cum).
    invest_amount = cash amount to spend at next open.
    """
...
def print_stats(stats, initial_capital, equity_series, hold_pct, out_paths):
    """Prints backtest summary to stdout."""
    print('Scale-in + Hold backtest summary:')
    print(f'Initial capital:          {initial_capital:,.2f}')
    print(f'Hold fraction:            {hold_pct:.0%}')
...
def save_plots(df, equity_series, alloc_pct_list, num_pos_list, open_trades, stats, exit_sma_window, out_dir):
    ...
    ax_price.set_title('Price with Trades - Scale-in + Hold')
    ...
    fig.tight_layout()
    out_fig = os.path.join(out_dir, 'backtest_scalein_hold.png')
    fig.savefig(out_fig)
    plt.close(fig)
...
def run_backtest_scalein_hold(ticker='QQQ', period='5y', initial_capital=10000.0,
                               hold_pct=0.5,
    ...
    if df.empty:
        raise ValueError('No data returned from fetch_data — check ticker/period.')
    df = df.copy()
    ...
    hold_shares = 0.0       # permanently held, never sold
    open_trades = []
    closed_trades = []
    ...
    for i in range(len(df)):
        ...
        # buy hold position on the first bar after warmup
        if warmup_done and not hold_bought:
            if i + 1 < len(df):
                next_open = float(df['Open'].iat[i + 1])
                hold_amount = hold_pct * initial_capital
                hold_shares = hold_amount / next_open
                cash -= hold_shares * next_open
                hold_bought = True

        # compute equity (hold + dynamic positions + cash)
        hold_value = hold_shares * close
        dyn_value = sum(t['shares'] * close for t in open_trades)
        eq = cash + hold_value + dyn_value
        equity_list.append(eq)
        equity_dates.append(date)
        alloc_pct_list.append(((hold_value + dyn_value) / eq) if eq > 0 else 0.0)
        num_pos_list.append(len(open_trades))

        # check exits for dynamic trades only
        for t in open_trades[:]:
        ...
        # check entry signal for dynamic portion
    ...
    fig_path = None
    if verbose:
        trades_csv = os.path.join(out_dir, 'trades_detail_scalein_hold.csv')
        ...
        print_stats(stats, initial_capital, equity_series, hold_pct, out_paths)
...
def grid_search(ticker='QQQ', period='5y', initial_capital=10000.0,
    ...
    if raw_df.empty:
        raise ValueError('No data returned — check ticker/period.')

    results = []
    for idx, (hold_pct, bin_size, max_drop, init_alloc, sma_win) in enumerate(combos, 1):
        if bin_size >= max_drop:
            continue
        try:
            res = run_backtest_scalein_hold(
            ...
            print(f'  [{idx}/{total}] ERROR {hold_pct},{bin_size},{max_drop},{init_alloc},{sma_win}: {e}')
    ...
    print(f'  hold_pct={best["hold_pct"]}, bin_size_pct={best["bin_size_pct"]}, '
          f'max_drop_pct={best["max_drop_pct"]}, initial_alloc_pct={best["initial_alloc_pct"]}, '
          f'exit_sma_window={int(best["exit_sma_window"])}')
    print('Running full backtest with best parameters...\n')
    run_backtest_scalein_hold(
        initial_capital=initial_capital,
        hold_pct=best['hold_pct'],
...
if __name__ == '__main__':
    grid_search()
    #res = run_backtest_scalein_hold(initial_capital=10000.0, hold_pct=0.5, bin_size_pct=0.02, max_drop_pct=0.2, initial_alloc_pct=0.3, exit_sma_window=30)
    #print('\nRESULT:', res)
